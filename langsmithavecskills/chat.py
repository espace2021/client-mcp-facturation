...
from load_skills import verifier_outils_skills
...
async def get_agent() :
    ...
    # code pour vérifier les outils attendus par les skills
    manquants = verifier_outils_skills(SKILLS, outils)
    if manquants:
        for nom_skill, noms in manquants.items():
            print(
                f"  Outils attendus par la skill {nom_skill!r} mais absents "
                f"du serveur MCP : {noms}"
            )
        sys.exit(1)
